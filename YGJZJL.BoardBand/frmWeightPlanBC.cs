...
namespace YGJZJL.BoardBand
{
    ...
    {
        private const string CNST_BC_STATION_A = "K23";         //中宽带卷取秤1#
        private const string CNST_BC_STATION_B = "K24";         //中宽带卷取秤2#
        private const string CNST_BC_STATION_C = "K25";         //中宽带地上衡
        ...
        private void GetPlanInfo(string strBatchNo)
        {
            ...
            if (cbEdt_Status.SelectedIndex > 0)
            {
                ...
                {
                    strWhere += Convert.ToString("   and exists (select 1 from dt_zkd_plan t1 where t1.fs_batchno = t.fs_zc_batchno and t1.fs_pointid is not null and t1.fn_bandcount > 0)").Trim() + " ";
                }
                else if (cbEdt_Status.Value.Equals("0"))
                {
                    strWhere += Convert.ToString("   and not exists (select 1 from dt_zkd_plan t1 where t1.fs_batchno = t.fs_zc_batchno and t1.fs_pointid is not null and t1.fn_bandcount > 0)").Trim() + " ";
                }
            }
            //完炉状态
            if (cbEdt_Finish.SelectedIndex > 0)
            {
                ...
                {
                    strWhere += Convert.ToString("   and exists (select 1 from dt_zkd_plan t1 where t1.fs_batchno = t.fs_zc_batchno and t1.fs_pointid is not null and t1.fs_completeflag = '1')").Trim() + " ";
                    strWhere += Convert.ToString("   and not exists (select 1 from dt_zkd_plan t1 where t1.fs_batchno = t.fs_zc_batchno and t1.fs_pointid is not null and t1.fs_completeflag = '0')").Trim() + " ";
                }
                else if (cbEdt_Finish.Value.Equals("0"))
                {
                    strWhere += Convert.ToString("   and exists (select 1 from dt_zkd_plan t1 where t1.fs_batchno = t.fs_zc_batchno and t1.fs_pointid is not null and t1.fs_completeflag = '0')").Trim() + " ";
                    strWhere += Convert.ToString("   and not exists (select 1 from dt_zkd_plan t1 where t1.fs_batchno = t.fs_zc_batchno and t1.fs_pointid is not null and t1.fs_completeflag = '1')").Trim() + " ";
                }
            }
            ...
            strSql += Convert.ToString("          from dt_zkd_plan t").Trim() + " ";
            ...
            strSql += Convert.ToString("          from dt_zkd_productweightdetail t").Trim() + " ";
            strSql += Convert.ToString("         where t.fs_batchno = fs_zc_batchno").Trim() + " ";
            ...
            strSql += Convert.ToString("          from dt_zkd_plan t").Trim() + " ";
            strSql += Convert.ToString("         where t.fs_batchno = fs_zc_batchno").Trim() + " ";
            strSql += Convert.ToString("           and t.fs_pointid = '" + CNST_BC_STATION_C + "') fn_point_02_plan,").Trim() + " ";
            ...
            strSql += Convert.ToString("          from dt_zkd_productweightdetail t").Trim() + " ";
            strSql += Convert.ToString("         where t.fs_batchno = fs_zc_batchno").Trim() + " ";
            strSql += Convert.ToString("           and t.fs_point = '" + CNST_BC_STATION_C + "') fn_point_02_num,").Trim() + " ";

            strSql += Convert.ToString("       (select sum(nvl(t.fn_weight, 0))").Trim() + " ";
            strSql += Convert.ToString("          from dt_zkd_productweightdetail t").Trim() + " ";
            strSql += Convert.ToString("         where t.fs_batchno = fs_zc_batchno").Trim() + " ";
            strSql += Convert.ToString("           and t.fs_point = '" + CNST_BC_STATION_C + "') fn_point_02_wgt,").Trim() + " ";

            strSql += Convert.ToString("       (select sum(nvl(t.fn_theoryweight, 0))").Trim() + " ";
            strSql += Convert.ToString("          from dt_zkd_productweightdetail t").Trim() + " ";
            strSql += Convert.ToString("         where t.fs_batchno = fs_zc_batchno").Trim() + " ";
            strSql += Convert.ToString("           and t.fs_point = '" + CNST_BC_STATION_C + "') fn_point_02_wgt_ll,").Trim() + " ";

            strSql += Convert.ToString("       (select decode(t.fs_completeflag, '1', '√', '')").Trim() + " ";
            strSql += Convert.ToString("          from dt_zkd_plan t").Trim() + " ";
            strSql += Convert.ToString("         where t.fs_batchno = fs_zc_batchno").Trim() + " ";
            strSql += Convert.ToString("           and t.fs_pointid = '" + CNST_BC_STATION_C + "') fn_point_02_done,").Trim() + " ";
            ...
            strSql += Convert.ToString("                        (select sum(nvl(t1.fn_weight,0)) from dt_zkd_productweightdetail t1 where t1.fs_batchno = t.fs_zc_batchno) rate_actual,").Trim() + " ";
            strSql += Convert.ToString("                        (select sum(nvl(t1.fn_theoryweight,0)) from dt_zkd_productweightdetail t1 where t1.fs_batchno = t.fs_zc_batchno) rate_theory,").Trim() + " ";
            ...
            strSql += Convert.ToString("  from dt_zkd_plan t").Trim() + " ";
            strSql += Convert.ToString(" where t.fs_gp_flow = 'SH000100'").Trim() + " ";
        ...
        private void btnDone_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (ultraOptionSet1.CheckedIndex)
                {
                    case 0: V1 = CNST_BC_STATION_A; break;
                    case 1: V1 = CNST_BC_STATION_B; break;
                    case 2: V1 = CNST_BC_STATION_C; break;
                    default: V1 = CNST_BC_STATION_A; break;
                }
                string V2 = this.UserInfo.GetUserName();

                if (MessageBox.Show("确认轧制编号【" + V0 + "】在 " + V1 + " 计量结束吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    return;

                string strProcedure = "KG_MCMS_FLOWCARD.WeightPlanBC_Done1", strErr = "";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (V2 == "2")
                {
                    if (MessageBox.Show("确认要删除轧制编号【" + V0 + "】在" + V1 + "的预报信息吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                        return;
                ...
                string strProcedure = "TEST.SaveWeightPlanZKD", strErr = "";
                object[] sArgs = new object[] { V0, V1, V2, V3, V5, V6, V7, V8, V17 };
