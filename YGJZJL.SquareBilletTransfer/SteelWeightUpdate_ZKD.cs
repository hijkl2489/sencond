...
{
    public partial class SteelWeightUpdate_ZKD :CoreFS.CA06.FrmBase
    {
       ...
       //}
        ...
        private void DoHandSaveMainForGP()
        {
            try
            {
                ...
                ccp.ServerName = "com.dbComm.DBComm";
                ccp.MethodName = "executeProcedureBySql";
                ccp.ServerParams = new object[] { "{call HG_MCMS_SQUAREBITTET.ADD_BOARDWEIGHT_MAIN(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}", ht };
                this.ExecuteNonQuery(ccp, CoreInvokeType.Internal);
                string result = "";
        ...
        private void UpdateStoveno()//修改炉号
        {
            ...
            if (MessageBox.Show("此操作会彻底修改该块钢坯的炉号，确定？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                return;
            }
            ...
            try
            {
                ...
                ccp.ServerName = "com.dbComm.DBComm";
                ccp.MethodName = "executeProcedureBySql";
                ccp.ServerParams = new object[] { "{call HG_MCMS_SQUAREBITTET.UpdateStoveno(?,?,?)}", ht };
                this.ExecuteNonQuery(ccp, CoreInvokeType.Internal);
                string result = "";
                ...
                if (ccp.ReturnObject != null)
                {
                    ...
                    this.m_BaseInfo_1.WriteOperationLog("DT_BOARDWEIGHTMAIN", strDepartMent, strUserName, "修改炉号", strLog, "板坯炉号修改", "板坯计量表、工艺流动卡表、板坯与报表", "板坯炉号修改");//调用写操作日志方法
        ...
        private void DeleteSteelWeightMain()
        {
            ...
            if (MessageBox.Show("此操作会彻底删除该块钢坯的计量数据，确定？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                return;
            }
            ...
            try{
             Hashtable ht = new Hashtable();
                ...
                ccp.ServerName = "com.dbComm.DBComm";
                ccp.MethodName = "executeProcedureBySql";
                ccp.ServerParams = new object[] { "{call HG_MCMS_SQUAREBITTET.DELETE_BOARDWEIGHT_MAIN(?,?)}", ht };
                this.ExecuteNonQuery(ccp, CoreInvokeType.Internal);
                string result = "";
            ...
            ////删除主表记录
            //string strSql = "delete from DT_BOARDWEIGHTMAIN where FS_STOVENO='" + strStoveNo + "'";
            //CoreClientParam ccp = new CoreClientParam();
            //ccp.ServerName = "ygjzjl.basedatamanage.OtherBaseInfo";
            //ccp.MethodName = "ExcuteNonQuery";
            //ccp.ServerParams = new object[] { strSql };
