...
{
    public partial class ZkdBpSap : FrmBase
    {
        #region 变量定义
        ...
        private void CancelUpdate()
        {
            ...
            if (uGridData.Selected.Rows.Count > 0)
            {
                foreach (UltraGridRow ugr in uGridData.Selected.Rows)
                {
                    ...
                    if (sapClass.uptData(strTmpTable, strTmpField, strTmpWhere))
                    {
                        lstHint2.Items.Add("撤销修改成功！");
                    }
                    else
                    {
                        MessageBox.Show("撤销修改失败！");
                    }
                }
            }
            else
            {
                MessageBox.Show("请先选择要撤销修改的数据行！");
            }
        }
        ...
        private bool getStrWhere()
        {
            ...
            if (qTxtLh.Text.Trim() != "")
            {
                strWhere += " AND FS_STOVENO = '" + qTxtLh.Text + "'";
            }
            ...
            for (int i = 1; i <= num; i++)
            {
                ...
                else
                {
                   ...
                   // val13 = values.GetValue(i, 13).ToString().Trim(); //特殊库存标志
                    ...
                    strTmpField = "FS_ACCOUNTDATE,FS_PRODUCTNO,FS_ITEMNO,FS_STOVENO,FN_TOTALWEIGHT,FS_PLANT,FS_SAPSTORE,FS_HEADER,FD_STARTTIME,FS_COMPLETEFLAG";
                    strTmpValue = "'" + val1 + "','" + val2 + "','" + val3 + "','" + val6 + "','" + val7 + "','" + val9+ "','" 
                        + val10 + "','" + val16 + "',sysdate,'1'";
        ...
        private void showGridInfo()
        {
           ...
           // String FN_NETWEIGHT = "";
            ...
            strTmpField = "TO_CHAR(t.FD_WEIGHTTIME,'yyyy-MM-dd HH24:mi:ss') AS GBSJ,t.FS_STOVENO,t.FS_PRODUCTNO,t.FS_ACCOUNTDATE,t.FS_MATERIALNAME,t.FS_STEELTYPE,t.FS_SPEC,t.FS_AUDITOR,t.FS_ITEMNO,t.FS_PLANT,t.FS_SAPSTORE,t.FS_UPLOADFLAG,t.FS_ISMATCH,";

            strTmpField += " fn_weight";
        ...
        private void txtLh_Leave(object sender, EventArgs e)
        {
            ...
            if (txtLh.Text.Trim().Length > 12)
            {
                MessageBox.Show("炉号不能大于12位，请重新进行输入！");
                txtLh.Focus();
            }
        }
        ...
        private void upLoadData()
        {
            ...
            strTmpMater = "";
            string strTmpNo = "", strTmpBatch1, strTmpBatch2;
            decimal dTmpZl = 0;
            ...
            for (int i = 0; i < tmpRow.Length; i++)
            {
                ...
                int length = tmpRow[i]["FS_STOVENO"].ToString().Length ;
                strTmpBatch1 = tmpRow[i]["FS_STOVENO"].ToString();
                strTmpBatch2 = (i < (tmpRow.Length - 1)) ? tmpRow[i + 1]["FS_STOVENO"].ToString() : tmpRow[i]["FS_STOVENO"].ToString();
               ...
               // if (!chkDataInfo(i)) continue;
                if ((strTmpBatch1 != strTmpBatch2) || (i == (tmpRow.Length - 1)))
                {
                  ...
                  //  strTmpNo = tmpRow[i]["FS_STOVENO"].ToString();
                    ...
                    if (sTmp != "")
                    {
                        //  sapClass.strArrayUpload[2] = sTmp + "-" + tmpRow[i]["FS_STOVENO"].ToString();
                        sapClass.strArrayUpload[2] = sTmp + "-" + strTmpNo;
                        sapClass.strArrayUpload[6] = dTmpZl.ToString();
                        //  sTmp = "炉号为" + tmpRow[i]["FS_STOVENO"].ToString() + "-凭证号" + sTmp;
                        sTmp = "炉号为" + strTmpNo + "-凭证号" + sTmp + "-上传重量" + dTmpZl.ToString();
                        //  sapClass.ins261Data(strTmpMater, dTmpZl.ToString(), strTmpNo, uGridData.Rows[i].Cells["FS_SAPSTORE"].ToString());
                        //sapClass.uptDataFlag("DT_SteelWeightMain", "FS_UPLOADFLAG='1'", "FS_STOVENO='" + strTmpNo + "'");
                        //string a = strTmpNo.Insert(9, "-");
                        sapClass.uptDataFlag("dt_boardweightmain", "FS_UPLOADFLAG='1'", "FS_STOVENO = '" + strTmpBatch1 + "'");
                        sapClass.uptSapLog(strTmpBatch1, sapClass.strArrayUpload);
                        sTmp += "，数据上传成功！";
