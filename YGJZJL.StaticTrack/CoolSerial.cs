    ...
    class CoolSerial
    {
        ...
        private void Parse_XK3124()
        {
            ...
            if (m_SerialPort.BytesToRead > 0)
            {
                ...
                //=和=之间是数据
                //strRecv.Replace("-", "0");
                int index = strRecv.ToString().LastIndexOf((char)0x02);

                if (strRecv.Length > index+9)//index >= 16
                {
                    strData = strRecv.ToString().Substring(index+4,6).Trim();
                    //strData.Trim();
                    //byte[] buff = Encoding.ASCII.GetBytes(strData);
                    //Array.Reverse(buff);
                    m_szData = strData;
                    m_dData = Decimal.Parse(strData) / 1000;
                    //m_dData = getWeight_XK3190(strData);
                    strRecv.Remove(0, index + 1);
                }
            }
