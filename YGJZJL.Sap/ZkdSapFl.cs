...
{
    public partial class ZkdSapFl : FrmBase
    {
        #region 变量定义
        ...
        private bool getStrWhere()
        {
         ...
         //   strWhere += " AND FS_DRDW='棒线厂四作'";
            ...
            strWhere += " AND FS_SAPSTORE = '1310'";//玉钢中宽带库
            return true;
        }
        ...
        private void upLoadData()
        {
            ...
            for (int i = 0; i < uGridData.Rows.Count; i++)
            {
                ...
                if (uGridData.Rows[i].Cells["FS_UPLOADFLAG"].Value.ToString() == "0")
                {
                    ...
                    strStoveno = uGridData.Rows[i].Cells["FS_STOVENO"].Value.ToString();
                    strTmpNo = strStoveno;
                    dTmpZl = Convert.ToDecimal(uGridData.Rows[i].Cells["FN_NETWEIGHT"].Value);

                    strCovertStoveno = strTmpNo.Split('-');
                    ...
                    if (sTmp != "")
                    {
                        ...
                        sapClass.uptDataFlag("DT_SAP261", strTmpValue, "FS_STOVENO='" + strStoveno
