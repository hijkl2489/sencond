...
{
    public partial class PredictInfo : FrmBase
    {
        ...
        private bool MoreNonCardYBDelete()
        {
            ...
            if (dtServerYB.Rows.Count > 0)
            {
                DialogResult dr= MessageBox.Show("系统中已存在该车号的预报，确认覆盖请点击确定！","提示！", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (dr == DialogResult.OK)
            {
                for (int i = 0; i < dtServerYB.Rows.Count; i++)
                {
        ...
        private void QueryYBData()
        {
            ...
            sql += " from DT_WeightPlan where 1=1 and FS_DEPARTID in (select t.DEPARTID from CORE_APP_DEPARTMENT t ,CORE_APP_DEPARTMENT s where (t.PID = s.DEPARTID or t.PID = null) and t.DEPARTID = '" + str_DepartID + "') ";
