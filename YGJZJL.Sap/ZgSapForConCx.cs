...
{
    public partial class ZgSapForConCx : FrmBase
    {
        #region 变量定义
        ...
        private void showGridInfo()
        {
            ...
            strTmpTable = "dt_pipeweightmain t";
            ...
            strTmpField += " (select sum( r.FN_WEIGHT) from dt_pipeweightdetail r where r.FS_BATCHNO=t.FS_BATCHNO and r.FS_UPLOADFLAG='1') as  FN_TOTALWEIGHT,";
            strTmpField += "(select sum( r.FN_THEORYWEIGHT) from dt_pipeweightdetail r where r.FS_BATCHNO=t.FS_BATCHNO and r.FS_UPLOADFLAG='1')  as FN_THEORYTOTALWEIGHT";
        ...
        private void UpSap_Load(object sender, EventArgs e)
        {
          ...
          /*  if ((CoreFS.SA06.CoreUserInfo.UserInfo.GetRole() == "BXC1002") || 
            ...
            strWhere += " AND t.FS_SAPSTORE = '8500'";
            switch (strKey)
            {
                case "ZF":
                    strWhere += " AND  (FS_BATCHNO like '%TZ%' OR FS_BATCHNO LIKE '%TX%' ) ";
