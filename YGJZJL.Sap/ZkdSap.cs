...
{
    public partial class ZkdSap : BaseInfo
    {
        #region 变量定义
     ...
     /*   public BcSap(object obb)
        ...
        private void doUpdate(int iFlag)
        {
            ...
            if (uGridData.Selected.Rows.Count > 0)
            {
                foreach (UltraGridRow ugr in uGridData.Selected.Rows)
                {
                    ...
                    p_FS_MATERIAL = getMaterial(p_FS_PRODUCTNO);
                    ...
                    if (iFlag == 0)
                    {
                        ...
                        strTmpField += ",FS_ISMATCH='1',FS_MATERIALNO='" + p_FS_MATERIAL + "',FS_MATERIALNAME='"+p_FS_MATERIALNAME+"'";
        ...
        private bool getStrWhere()
        {
            ...
            strWhere += " AND FS_DELETEFLAG != '1'";
            //strWhere += " AND FS_COMPLETEFLAG = '1'";

            //strWhere += " AND FS_SAPSTORE = '1310'";//玉钢棒材库存地
       ...
       /*     if ((CoreFS.SA06.CoreUserInfo.UserInfo.GetRole() == "BXC1002") || 
        ...
        private void showGridInfo()
        {
            ...
            strTmpTable = "view_zkd_data";
            strTmpField = "TO_CHAR(FD_DATETIME,'yyyy-MM-dd HH24:mi:ss') AS GBSJ,FS_BATCHNO,FN_BANDNO,FS_PRODUCTNO,"
               + "FS_PLANT,FS_SAPSTORE,FS_UPLOADFLAG,FN_YKL ,FN_KHJZ,FN_WEIGHT,FS_ACCOUNTDATE,FS_STEELTYPE,FS_SPEC,FS_ISMATCH,"
               + "FS_HEADER,FS_GP_STOVENO,FN_JJ_WEIGHT,FS_MATERIALNAME";
    ...
    /*    private void uptData(int iFlag)
        ...
        private void UpSap_Load(object sender, EventArgs e)
        {
            ...
            strWhere += " AND FS_DELETEFLAG != '1'";
            //strWhere += " AND FS_COMPLETEFLAG = '1'";
            //strWhere += " AND FS_SAPSTORE = '1310'";//玉钢型材库存地
              ...
              //  if (tmpRow[i].Cells["FS_ISMATCH"].Value.ToString() == "0") continue;
               // if (!chkDataInfo(i)) continue;
                ...
                if (ckTheory.Checked)
                    ...
                    dTmpZl += Convert.ToDecimal(tmpRow[i]["FN_KHJZ"]);
                ...
                if ((strTmpBatch1 != strTmpBatch2) || (i == (tmpRow.Length - 1)))
                {
                    ...
                    if (ccp.ReturnCode == 0)
                    {
                       ...
                       // downSprueflosInfo(sTmp, strTmpNo);
                        ...
                        sapClass.uptDataFlag("dt_zkd_productweightmain", "FS_UPLOADFLAG='1',FS_MATERIALNO='" + strTmpMater + "'", "FS_BATCHNO='" + strTmpNo
                         + "' AND FS_ACCOUNTDATE='" + strTmpJzrq + "'");

                        for (int j = 0; j < listBatch.Count; j++)
                        {
                            sapClass.uptDataFlag("dt_zkd_productweightdetail", strTmpValue, "FS_BATCHNO='" + strTmpNo
