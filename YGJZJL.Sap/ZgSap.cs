...
{
    public partial class ZgSap : BaseInfo
    {
        #region 变量定义
     ...
     /*   public BcSap(object obb)
        ...
        private bool getStrWhere()
        {
            ...
            strWhere += " AND FS_UPLOADFLAG = '0' AND FS_POINT IN ('K26','K27')";
       ...
       /*     if ((CoreFS.SA06.CoreUserInfo.UserInfo.GetRole() == "BXC1002") || 
        ...
        private void showGridInfo()
        {
            ...
            strTmpField = "TO_CHAR(FD_DATETIME,'yyyy-MM-dd HH24:mi:ss') AS GBSJ,FS_BATCHNO,FN_BANDNO,FS_PRODUCTNO,FS_MATERIALNAME,"
               + "FS_PLANT,FS_SAPSTORE,FS_UPLOADFLAG,FN_WEIGHT,FN_THEORYWEIGHT,FS_ACCOUNTDATE,FS_STEELTYPE,FS_SPEC,FS_ISMATCH,"
               + "FS_HEADER,FS_PRIBATCHNO ";
        ...
        private void ckTheory_CheckedChanged(object sender, EventArgs e)
        {
            if (ckTheory.Checked)
            {
                if (DialogResult.No == MessageBox.Show("您将按理论重量上传数据，是否继续？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information))
                {
