...
namespace YGJZJL.CarSip.Client.Meas
{
    ...
    public class CoreWeight : CoreSieralPort
    {
        ...
        #region <成员变量>
        protected string _beginFlag;           // 开始标志
        protected string _endFlag;             // 结束标志
        ...
        protected double _precision = 0.001;
        ...
        public override void Init(string config)
        {
            ...
            if (strtmp.Length > 11)
            {
                _precision = Convert.ToDouble(strtmp[11]);                
            }
            if (strtmp.Length > 12)
            {
...
//            }
        ...
        public void OnDataReceived()
        {
            ...
            if (SerialPort != null && SerialPort.IsOpen)
            {
                try
                {
                    _strBuf += SerialPort.ReadExisting();//SerialPort.ReadLine();
                    if (!_receiveFlag)
                    {
                        _strBuf = "";
                        return;
                    ...
                    {
                        curPos = _strBuf.IndexOf(EndFlag, prePos);
                        if (-1 == curPos)
                        {
                            _strBuf = _strBuf.Substring(prePos);
                            break;
                        }
                        data = _strBuf.Substring(prePos, curPos-prePos);
                        if (data.Length == _msgLen - EndFlag.Length)
                        {
                            ...
                            {
                                double weight = Double.Parse(strWeight)/1000;
                                HandleWeight(weight);
                            }
                        }
