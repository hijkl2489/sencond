...
namespace YGJZJL.StaticTrack
{
    ...
    public class ProductPoundRoom
    {
        ...
        static string m_szRunPath = System.Environment.CurrentDirectory.ToString();
            ...
            try
            {
                ...
                while (m_bRunning)
                {
                    ...
                    if (UseMeter)
                    {
                        MeterData = m_CoolSerialForMeter.StringData;
                        MeterValue = m_CoolSerialForMeter.DecimalData;
                        strLog = "串口数据：" + MeterData + " 显示数据：" + MeterValue;
                        WriteLog(strLog);
                    }
                    ...
                    System.Threading.Thread.Sleep(500);//请根据需要调整线程运行周期  上次是100ms
                }
            }
