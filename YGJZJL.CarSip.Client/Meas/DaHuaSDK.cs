 ...
 *
 * 文件名称:DaHuaSDK.cs
 * 功能说明:在现有的大华网络SDK上再一次封装，针对C#应用开发
 * 作    者:宾洪斌
 * 创建时间:2012-03-15
 * 更新日志:    日期        版本号      作者        变更原因
 *              2012-03-15  1.0         宾洪斌      新建
 * 
 * ************************************************************************
 * (c) Copyright 2012, Hunnan  ShiTuo Technology Developement Co.Ltd. All Rights Reserved
 */
...
namespace DHNetSDK
{
    ...
    public sealed class DHClient
    {
        ...
        /// <param name="dwUserData">用户自定义数据</param>
        /// <returns>成功返回网络回放ID，失败返回0</returns>
        public static int DHPlayBackByTime(int lLoginID, int nChannelID, DateTime tmStart, DateTime tmEnd, IntPtr hWnd, fTimeDownLoadPosCallBack cbDownLoadPos, IntPtr dwUserData)
        {
    ...
    public delegate void fTimeDownLoadPosCallBack(int lPlayHandle, UInt32 dwTotalSize, UInt32 dwDownLoadSize,int nindex, NET_RECORDFILE_INFO recordinfo, IntPtr dwUser);
