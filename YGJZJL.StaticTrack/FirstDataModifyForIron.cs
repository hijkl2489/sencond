...
{
    public partial class FirstDataModifyForIron: FrmBase
    {
        /// <summary>
        /// 临时生产单号
        /// </summary>
        ...
        private void QueryData()
        {
            ...
            string sql = "SELECT FS_STOVESEATNO,FS_STOVENO,FN_WEIGHT as fn_grossweight ,"
                       + " FS_WEIGHTNO,'FALSE' as XZ,FS_POTNO,"

                        + "  to_char(FD_WEIGHTTIME,'yyyy-mm-dd hh24:mi:ss') as FS_GROSSTIME,"
                       + "decode(FS_WEIGHTTYPE,'1','炼钢','2','铸铁','炼钢') as FS_RECEIVEFACTORY1"
                       + " ,FS_WEIGHTPERSON as fs_grossperson "
                       + " FROM DT_STATICTRACKFIRSTWEIGHT "
                        + " WHERE FS_WEIGHTPOINT='" + PointID + "' AND (FD_WEIGHTTIME BETWEEN to_date('" + strBeginDate + "','yyyy-MM-dd hh24:mi:ss') and to_date('" + strEndDate + "','yyyy-MM-dd hh24:mi:ss') "
        ...
        private void DeleteRecord()
        {
            ...
            string sql = "delete DT_STATICTRACKFIRSTWEIGHT where FS_WEIGHTNO='" + strWeightNoTemp + "'";
            ccp.ServerParams = new object[] { sql };
            this.ExecuteNonQuery(ccp, CoreInvokeType.Internal);
          ...
          + strStoveSeatNo + ",炉号=" + strStoveNo + ",毛重="//物料、发货单位
          + strGrossWeight +  ",流向=" + strReceiveFac + ",";
                this.objBi.WriteOperationLog("DT_STATICTRACKFIRSTWEIGHT", strDepartMent, strUserName, "删除", strLog, "数据修改", "静态铁水衡一次数据表", "静态铁水衡");//调用写操作日志方法
        ...
        private void UpdateData()
        {
            ...
            //{
                ...
                ccp.ServerName = "ygjzjl.basedatamanage.OtherBaseInfo";
                ccp.MethodName = "ExcuteNonQuery";
                string sql = " UPDATE DT_STATICTRACKFIRSTWEIGHT "
                             + "SET FS_STOVESEATNO='" + strStoveSeatNo + "',"
                             + "FS_STOVENO='" + strStoveNo + "',"
                             + "FS_POTNO='" + strPotNo + "',"
                             + "FN_WEIGHT='" + strGrossWeight + "',"
            ...
            string strLog = "罐号：" + strPotNoLog + ">" + strPotNo + ",炉座号："//卡号、车号
                ...
                + strGrossWeightLog + ">" + strGrossWeight + ","//卸货地点、承运单位
                + strReceiveFacLog + ">" + strReceiveFac ;//流向、重量
            this.objBi.WriteOperationLog("DT_STATICTRACKFIRSTWEIGHT", strDepartMent, strUserName, "修改", strLog, "数据修改", "静态铁水衡一次数据表", "静态铁水衡");//调用写操作日志方法
