...
{
    public partial class ZgSapForConExcel : FrmBase
    {
        #region 变量定义
        ...
        private void doUpdate()
        {
            ...
            if (uGridData.Selected.Rows.Count > 0)
            {
                foreach (UltraGridRow ugr in uGridData.Selected.Rows)
                {
                    ...
                    p_FS_ITEMNO = (txtHxmh.Text != "") ? txtHxmh.Text : uGridData.Rows[ugr.Index].Cells["FS_ITEMNO"].Value.ToString();
                    p_FS_MATERIALNAME = "";
                    ...
                    strTmpField = "FS_PRODUCTNO='" + txtScdd.Text + "',FS_ITEMNO='0001',FS_SAPSTORE='" + txtKcd.Text + "',"
                        + "FS_ACCOUNTDATE='" + dteJzrq.Value.ToString("yyyy-MM-dd") + "',FS_PLANT='1100',fs_materialname='" + txtWlmc.Text + "'";
        ...
        private void upLoadData()
        {
            ...
            for (int i = 0; i < uGridData.Rows.Count; i++)
            {
                ...
                if (uGridData.Rows[i].Cells["FS_UPLOADFLAG"].Value.ToString() == "0")
                {
                    ...
                    strTmpMater = uGridData.Rows[i].Cells["FS_MATERIALNO"].Value.ToString();
                    strTmpNo = uGridData.Rows[i].Cells["FS_STOVENO"].Value.ToString();
                    strXmh = uGridData.Rows[i].Cells["FS_ITEMNO"].Value.ToString();
                    listItem.Clear();
                    listSubItem.Clear();
                    ...
                    //string sTmp = sapClass.BAPI_GOODSMVT_CREATE("01",strTmpHeader, listSubItem);
                    ...
                    if (sTmp != "")
                    {
                      ...
                      //  sapClass.ins261Data(strTmpMater, dTmpZl.ToString(), strTmpNo, uGridData.Rows[i].Cells["FS_SAPSTORE"].Value.ToString());
                        sapClass.uptDataFlag("DT_CARWEIGHT_WEIGHT", "FS_UPLOADFLAG='1'", "FS_STOVENO='" + strTmpNo + "'");
