...
{
    public partial class BilletInfo : FrmBase
    {
        #region 定义变量
        //校秤：
        ...
        private string pointcode = "K16";        //计量点代码,默认为辊道秤
        ...
        private string _ledIP = "10.32.7.176,0,6666,1025";
        ...
        private void btnBC_Click_1(object sender, EventArgs e)
        {
            ...
            double tmpWeight = 0;
            if (double.TryParse(txtXSZL.Text.Trim(), out tmpWeight))
            {
       ...
       //窗体初始化
        private void BilletInfo_Load(object sender, EventArgs e)
        {
        ...
        private void QueryAndBindJLCZData()
        {
            ...
            if (dataTable6.Rows.Count > 0 && dataTable6.Rows[0]["FS_THWEITSINGLE"].ToString() == "理论")
                str = "理论计重";
            if (dataTable1 != null && dataTable1.Rows != null && dataTable1.Rows.Count > 0 && dataTable3.Rows.Count>0)
            {
                ultraGroupBox1.Text = "本炉已计量： " + dataTable1.Rows[0]["FN_BILLETCOUNT"].ToString() + "/" + dataTable3.Rows[0]["FN_COUNT"] + "支，共计 " + dataTable1.Rows[0]["FN_TOTALWEIGHT"].ToString() + " 吨"+"     "+str;
                ultraGroupBox1.Refresh();
            }
            else
                ultraGroupBox1.Text = "本炉已计量数据";
        }
        ...
        private void QueryAndBindZBData()
        {
            ...
            ccp.ServerName = "com.dbComm.DBComm";
            ccp.MethodName = "query";
            ccp.ServerParams = new object[] { "BILLETINFO_03.SELECT", param };
            ccp.SourceDataTable = dataTable1;
            this.ExecuteQueryToDataTable(ccp, CoreInvokeType.Internal);
        }

        //查询计量从表表数据
        private void QueryAndBindCBData()
        {
            string strLH = this.txtLH.Text.Trim();
            ArrayList param = new ArrayList();
            param.Add(strLH);
            ...
            ccp.ServerName = "com.dbComm.DBComm";
            ccp.MethodName = "query";
            ccp.ServerParams = new object[] { "BILLETINFO_04.SELECT", param };
            ccp.SourceDataTable = dataTable6;
            this.ExecuteQueryToDataTable(ccp, CoreInvokeType.Internal);
            if (dataTable6.Rows.Count > 0)
            {
                for (int i = 0; i < dataTable6.Rows.Count; i++)
                {
                    if (dataTable6.Rows[i]["FS_THWEITSINGLE"].ToString() == "1")
                        dataTable6.Rows[i]["FS_THWEITSINGLE"] = "理论";
                    else
                        dataTable6.Rows[i]["FS_THWEITSINGLE"] = "正常";
                }
            }
        ...
        private void QueryAndBindJLDData()
        {
            ...
            ccp.ServerName = "com.dbComm.DBComm";
            ccp.MethodName = "query";
            ccp.ServerParams = new object[] { "BILLETINFO_05.SELECT",new ArrayList() };
        ...
        private void SavePictureData()
        {
            ...
            {
                strZL = dataTable6.Rows[0]["FS_STOVENO"] + ":\n" + dataTable6.Rows[0]["FN_BILLETINDEX"].ToString() + "/" + dataTable3.Rows[0]["FN_COUNT"] + "支\n" + dataTable6.Rows[0]["FN_NETWEIGHT"].ToString() + " 吨";
                ultraGroupBox1.Refresh();
            }
        ...
        {   
            ...
            ccp.ServerParams = new object[] { strLH, _rollerNo };
       ...
       */
        ...
        {
            ArrayList param = new ArrayList();
            param.Add(UserInfo.GetUserOrder());
            param.Add(UserInfo.GetUserGroup());
            param.Add(UserInfo.GetUserName());
            DataTable dt = new DataTable();
            CoreClientParam ccp = new CoreClientParam();
            ccp.ServerName = "com.dbComm.DBComm";
            ccp.MethodName = "query";
            ccp.ServerParams = new object[] { "BILLETINFO_21.SELECT", param };
            ccp.SourceDataTable = dt;
        ...
        {
            //保存按钮是否可用
            button18.Enabled = !chkAutoSave.Checked;
            textBox1.Enabled = !chkAutoSave.Checked;
            textBox1.Text = "";
        }
        ...
        {
            double weight = 0;
            if (double.TryParse(textBox1.Text.Trim(), out weight))
            {
                Save();
                QueryCount();
            }
            ...
            {
                MessageBox.Show("请输入正确的方坯重量！");
            }
        }

        private void BilletInfo_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (_measApp != null)
            {
                _measApp.Finit();
