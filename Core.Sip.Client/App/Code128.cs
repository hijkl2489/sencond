...
namespace Core.Sip.Client.App
{
    public class Code128
    {
        ...
        //打印制管标签条形码
        public void printPipeCode(string code, Rectangle rect,/* int X, int Y,*/ System.Drawing.Printing.PrintPageEventArgs e)
        {
            if (code.Trim() != "")
            {
                Bitmap saved = new Bitmap(rect.Width, rect.Height);
        ...
        public void printCarCode(string code, int X, int Y, int size, System.Drawing.Printing.PrintPageEventArgs e)
        {
            if (code.Trim() != "")
            {
                ...
                if (size == 0) //二次重量
                {
                    e.Graphics.DrawImage(saved, 50, 429, X, Y);
                }
                if (size == 1) //一次重量
                {
                    e.Graphics.DrawImage(saved, 50, 389, X, Y);
                }
                if (size == 2) //外协
                {
                    e.Graphics.DrawImage(saved, 50, 209, X, Y);
                }
                if (size == 3) //钢坯二次
                {
                    e.Graphics.DrawImage(saved, 50, 499-25, X, Y);
                }
                if (size == 4) //钢坯一次
                {
                    e.Graphics.DrawImage(saved, 50, 459, X, Y);
                }
            }
