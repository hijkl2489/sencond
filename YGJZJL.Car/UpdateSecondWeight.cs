...
{
    public partial class UpdateSecondWeight : FrmBase
    {
        ...
        double douSendGross, douSendTare, douSendNet;
        ...
        private void Query()
        {
            ...
            sql += " A.FN_SENDGROSSWEIGHT,A.FN_SENDTAREWEIGHT,A.FN_SENDNETWEIGHT ";
        ...
        private void Update()
        {
            ...
            baseinfo.ob = this.ob;
            this.baseinfo.WriteOperationLog("DT_CARWEIGHT_WEIGHT", strDepartMent, strUserName, "修改", strLog, "历史数据修改", "汽车衡二次数据表", "汽车衡");//调用写操作日志方法
        ...
        private bool IsValid()
        {
            ...
            if (this.tb_sendGross.Text != "" && this.tb_sendTare.Text != "")
            {
                try
                {
                   ...
                   douSendGross = Math.Round(Convert.ToDouble(this.tb_sendGross.Text.Trim()), 3);
                   douSendTare = Math.Round(Convert.ToDouble(this.tb_sendTare.Text.Trim()), 3);
                   douSendNet = Math.Round((douSendGross - douSendTare), 3);
