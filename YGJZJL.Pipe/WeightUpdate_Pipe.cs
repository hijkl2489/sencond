...
{
    public partial class WeightUpdate_Pipe : CoreFS.CA06.FrmBase
    {
        ...
        private void WriteLog(string str)
        {
            ...
            System.IO.TextWriter tw = new System.IO.StreamWriter(System.Environment.CurrentDirectory + "\\log\\PipeWeight_" + strDate + ".log", true);
        ...
        private void QueryBCWeightDetailData(string strBatchNo)
        {
            try
            {
                string strSql = "select FS_WEIGHTNO,FS_BATCHNO,FN_BANDNO,FN_HOOKNO,FN_BANDBILLETCOUNT,FS_TYPE,FN_LENGTH,FN_WEIGHT,FS_PERSON,FS_POINT,FD_DATETIME,FS_SHIFT,FS_TERM,FS_LABELID,FN_DECWEIGHT,FS_FLAG,FS_UPFLAG,FN_THEORYWEIGHT,FS_RKDH from dt_pipeweightdetail where ";
            ...
            try
            {
                ...
                if (dt != null)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        string strSql = " select FS_STEELTYPE,FN_BANDCOUNT,FN_TOTALWEIGHT from dt_pipeweightmain where FS_BATCHNO = '" + p_FS_BATCHNO + "'";
                        ...
                        //if (m_Memo != "")
                           // m_BaseInfo.insertLog(strDate, "修改", p_UPDATER, strIP, strMAC, m_Memo, p_FS_BATCHNO, "", p_FS_BATCHNO, "", "", "dt_pipeweightmain", "棒材入库/计量数据维护");
                        Hashtable param = new Hashtable();
                        param.Add("I1",p_FS_BATCHNO);
                        param.Add("I2",p_FS_PRODUCTNO);
                        param.Add("I3",p_FS_STEELTYPE);
                        param.Add("I4",p_FS_SPEC);
                        param.Add("I5",p_FN_BANDCOUNT);
                        param.Add("I6",p_FN_BILLETCOUNT);
                        param.Add("I7",p_FN_TOTALWEIGHT);
                        param.Add("I8",p_FD_STARTTIME);
                        param.Add("I9",p_FD_ENDTIME);
                        param.Add("I10",p_FS_COMPLETEFLAG);
                        param.Add("I11",p_FS_UPLOADFLAG);
                        param.Add("I12",p_FN_THEORYTOTALWEIGHT);
                        param.Add("I13",p_UPDATER);
                        CoreClientParam ccp = new CoreClientParam();
                        ccp.ServerName = "com.dbComm.DBComm";
                        ccp.MethodName = "executeProcedureBySql2";
                        ccp.ServerParams = new object[] {"{call KG_MCMS_PIPE.UpdateWeightMain(?,?,?,?,?,?,?,?,?,?,?,?,?)}",param};
        ...
        private void DeleteBCWeightMain()
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                   ...
                   // m_BaseInfo.insertLog(strDate, "删除", p_UPDATER, strIP, strMAC, "", p_FS_BATCHNO, "", p_FS_BATCHNO, "", "", "dt_pipeweightmain", "棒材入库/计量数据维护");
                    Hashtable param = new Hashtable();
                    param.Add("I1", p_FS_BATCHNO);
                    param.Add("I2", p_FS_PRODUCTNO);
                    param.Add("I3", p_FS_STEELTYPE);
        ...
        private bool CheckBatchNoIsExist(string strBatchNo)
        {
            try
            {
                string strSql = "select * from dt_pipeweightmain where fs_batchno = '" + strBatchNo + "'";
        ...
        private void AddBCWeightMain()
        {
            try
            {
               ...
               // m_BaseInfo.insertLog(strDate, "增加", p_UPDATER, strIP, strMAC, "", p_FS_BATCHNO, "", p_FS_BATCHNO, "", "", "dt_pipeweightmain", "棒材入库/计量数据维护");
                Hashtable param = new Hashtable();
                param.Add("I1", p_FS_BATCHNO);
                param.Add("I2", p_FS_PRODUCTNO);
                param.Add("I3", p_FS_STEELTYPE);
        ...
        private bool IsUpLoadForMain(string strBatchNo, string strWeight, string strTheoryWeight)
        {
            ...
            try
            {
                string sql = "select FS_UPLOADFLAG,to_char(FN_TOTALWEIGHT,'FM99999.000') AS FN_TOTALWEIGHT,TO_CHAR(FN_THEORYTOTALWEIGHT,'FM99999.000') AS FN_THEORYTOTALWEIGHT from dt_pipeweightmain where FS_BATCHNO = '" + strBatchNo + "'";
                CoreClientParam ccp = new CoreClientParam();
                ccp.ServerName = "ygjzjl.basedatamanage.OtherBaseInfo";
                ccp.MethodName = "ExcuteQuery";
        ...
        private bool IsUpLoadForDetail(string strWeightNo, string strBatchNo, string strBandNo, string strWeight, string strTheoryWeight)
        {
            ...
            try
            {
                string sql = "select FS_UPLOADFLAG,FS_BATCHNO,FN_BANDNO,to_char(FN_WEIGHT,'FM99999.000') AS FN_WEIGHT,TO_CHAR(FN_THEORYWEIGHT,'FM99999.000') AS FN_THEORYWEIGHT from dt_pipeweightdetail where FS_WEIGHTNO = '" + strWeightNo + "'";
                CoreClientParam ccp = new CoreClientParam();
                ccp.ServerName = "ygjzjl.basedatamanage.OtherBaseInfo";
                ccp.MethodName = "ExcuteQuery";
                ccp.ServerParams = new object[] { sql };
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        string p_UPDATER = CoreFS.SA06.CoreUserInfo.UserInfo.GetUserName();
                        string p_RKDH = dt.Rows[i]["FS_RKDH"].ToString();
                        ...
                        string strSql = " select FS_BATCHNO,FN_BANDNO,FN_WEIGHT from dt_pipeweightdetail where FS_WEIGHTNO = '" + p_FS_WEIGHTNO + "'";
                        DataTable dts = new DataTable();
                        dts = m_BaseInfo.QueryData(strSql);
                      ...
                      //  if (m_Memo != "")
                            //m_BaseInfo.insertLog(strDate, "修改", p_UPDATER, strIP, strMAC, m_Memo, p_FS_WEIGHTNO, "", dts.Rows[0]["FS_BATCHNO"].ToString().Trim(), "", dts.Rows[0]["FN_BANDNO"].ToString().Trim(), "dt_pipeweightdetail", "棒材入库/计量数据维护");
                        Hashtable param = new Hashtable();
                        param.Add("I1", p_FS_WEIGHTNO);
                        param.Add("I2", p_FS_BATCHNO);
                        param.Add("I3", p_FN_BANDNO);
                        param.Add("I4", p_FN_BANDBILLETCOUNT);
                        param.Add("I5", p_FS_TYPE);
                        param.Add("I6", p_FN_LENGTH);
                        param.Add("I7", p_FN_WEIGHT);
                        param.Add("I8", p_FD_DATETIME);
                        param.Add("I9", p_FS_SHIFT);
                        param.Add("I10", p_FS_TERM);
                        param.Add("I11", p_FS_LABELID);
                        param.Add("I12", p_FS_UPFLAG);
                        param.Add("I13", p_FN_THEORYWEIGHT);
                        param.Add("I14", p_UPDATER);
                        param.Add("I15", p_RKDH);
                        CoreClientParam ccp = new CoreClientParam();
                        ccp.ServerName = "com.dbComm.DBComm";
                        ccp.MethodName = "executeProcedureBySql2";
                        ccp.ServerParams = new object[] { "{call KG_MCMS_PIPE.UpdateWeightDetail(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}", param };
        ...
        private string DeleteBCWeightDetail()
        {
            ...
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    string p_UPDATER = CoreFS.SA06.CoreUserInfo.UserInfo.GetUserName();
                    string p_RKDH = this.ultraGrid2.ActiveRow.Cells["FS_RKDH"].Text.ToString();
                    string strDate = System.DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
                    string strIP = m_BaseInfo.getIPAddress();
                    string strMAC = m_BaseInfo.getMACAddress();
                    //string p_UPDATER = CoreFS.SA06.CoreUserInfo.UserInfo.GetUserName();

                    //m_BaseInfo.insertLog(strDate, "删除", p_UPDATER, strIP, strMAC, "", p_FS_WEIGHTNO, "", p_FS_BATCHNO, "", p_FN_BANDNO, "dt_pipeweightdetail", "棒材入库/计量数据维护");
                    Hashtable param = new Hashtable();
                    param.Add("I1", p_FS_WEIGHTNO);
                    param.Add("I2", p_FS_BATCHNO);
                    param.Add("I3", p_FN_BANDNO);
            ...
            if (CheckIsNumber(this.tbx_RealWeight.Text.Trim()))
            {
                try
                {
                    if (this.ultraGrid2.Rows.Count == 0)
                    {
                        ...
                        string p_FS_PERSON =CoreFS.SA06.CoreUserInfo.UserInfo.GetUserName();
                        string p_FS_POINT = "";
                        p_FS_POINT = cbPoint.SelectedIndex < 0 ? "" : cbPoint.SelectedValue.ToString();
                        string p_FS_SHIFT = this.cbx_shift.SelectedValue.ToString().Trim();
                        string p_FS_TERM = this.cbx_term.SelectedValue.ToString().Trim();
                        ...
                        //m_BaseInfo.insertLog(strDate, "增加", p_UPDATER, strIP, strMAC, "", p_FS_WEIGHTNO, "", p_FS_BATCHNO, "", "", "dt_pipeweightdetail", "棒材入库/计量数据维护");
                        Hashtable param = new Hashtable();
                        param.Add("I1", p_FS_BATCHNO);
                        param.Add("I2", p_FS_PRODUCTNO);
                        param.Add("I3", p_FS_ITEMNO);
                        param.Add("I4", p_FS_MATERIALNO);
                        param.Add("I5", p_FS_MRP);
                        param.Add("I6", p_FS_STORE);
                        param.Add("I7", p_FS_STEELTYPE);
                        param.Add("I8", p_FS_SPEC);
                        param.Add("I9", p_FS_WEIGHTNO);
                        param.Add("I10", p_FN_BANDBILLETCOUNT);
                        param.Add("I11", p_FS_TYPE);
                        param.Add("I12", p_FN_LENGTH);
                        param.Add("I13", p_FN_WEIGHT);
                        param.Add("I14", p_FS_PERSON);
                        param.Add("I15", p_FS_POINT);
                        param.Add("I16", p_FS_SHIFT);
                        param.Add("I17", p_FS_TERM);
                        param.Add("I18", p_FS_LABELID);
                        param.Add("I19", p_FS_MATERIALNAME);
                        param.Add("I20", p_FS_FLOW);
                        param.Add("I21", p_FN_DECWEIGHT);
                        ...
                        if (this.ultraGrid1.ActiveRow.Cells["FS_COMPLETEFLAG"].Text == "1")
                        {
                            param.Add("I22", "");//入库单号
                            param.Add("O23", "");
                            ccp.ServerParams = new object[] { "{{call KG_MCMS_PIPE.Insert_PipeForHand(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}", param };
                        }
                        else
                        {
                            param.Add("I22", "");//吊号
                            param.Add("I23", "");//入库单号
                            ccp.ServerParams = new object[] { "{call KG_MCMS_PIPE.INSERT_DT_PIPEWEIGHT(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}", param };
                        }
                        CoreClientParam ret = this.ExecuteNonQuery(ccp, CoreInvokeType.Internal);
                        if (ret.ReturnCode != 0)
                        {
                    ...
                    else
                    {
                        ...
                        string p_FS_SHIFT = cbx_shift.SelectedValue.ToString().Trim();
                        string p_FS_TERM = cbx_term.SelectedValue.ToString().Trim();
                        string p_FS_LABELID = GetBarCode(p_FS_BATCHNO);
                        string p_FS_MATERIALNAME = "";
                        string p_FS_FLOW = "";
                        float p_FN_DECWEIGHT = 0;
                        ...
                        if (this.ultraGrid1.ActiveRow.Cells["FS_COMPLETEFLAG"].Text == "1")
                        {
                            param.Add("I22", tbRKDH.Text.Trim());//入库单号
                            param.Add("O23", "");
                            ccp.ServerParams = new object[] { "{{call KG_MCMS_PIPE.Insert_PipeForHand(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}", param };
                        }
                        else
                        {
                            param.Add("I22", "");//吊号
                            param.Add("I23", tbRKDH.Text.Trim());//入库单号
                            ccp.ServerParams = new object[] { "{call KG_MCMS_PIPE.INSERT_DT_PIPEWEIGHT(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}", param };
                        }
                        CoreClientParam ret = this.ExecuteNonQuery(ccp, CoreInvokeType.Internal);
                        if (ret.ReturnCode != 0)
        ...
        private string GetBarCode(string strBatchNo)
        {
            ...
            string sql = "select max(FN_BandNo) + 1 as bandNo from dt_pipeweightdetail where fs_batchno = '" + strBatchNo + "'";
            CoreClientParam ccp = new CoreClientParam();
            ccp.ServerName = "ygjzjl.basedatamanage.OtherBaseInfo";
            ccp.MethodName = "ExcuteQuery";
            ccp.ServerParams = new object[] { sql };
            ...
            string strBarCode = "402" + "N" + strYear + strBatchNo.Substring(2, 6) + strBandNo;
        ...
        private void BindShift()
        {
            ...
            dt.Rows.Add("2", "中");
            dt.Rows.Add("3", "晚");
            cbx_shift.DataSource = dt;
            cbx_shift.ValueMember = "Value";
            cbx_shift.DisplayMember = "Text";
        }
        ...
        private void BindPoint()
        {
            ...
            dt.Columns.Add("Value");
            dt.Columns.Add("Text");
            dt.Rows.Add("K26", "76机组");
            dt.Rows.Add("K27", "89机组");

            cbPoint.DataSource = dt;
            cbPoint.ValueMember = "Value";
            cbPoint.DisplayMember = "Text";
        }
        ...
        private void BindTerm()
        {
            ...
            dt.Rows.Add("3", "丙");
            dt.Rows.Add("4", "丁");
            cbx_term.DataSource = dt;
            cbx_term.ValueMember = "Value";
            cbx_term.DisplayMember = "Text";
        }
    }
}
